...
import { AttachmentBuilder, GuildMember, PermissionFlagsBits } from "discord.js";
            ...
            case "card-background":
              {
                try {
                  ...
                  if (!image) return await interaction.reply({
                    embeds: [
                  ...
                  await manager.prisma.userLevel.updateMany({
                    where: {
            ...
            case "set":
              {
                ...
                const existingConfig = await manager.prisma.levelConfig.findFirst({
                  where: { guildId: guildId },
                });

                if (existingConfig) {
                  await manager.prisma.levelConfig.update({
                  ...
                  await manager.prisma.levelConfig.create({
                    data: {
                      guildId: guildId,
            ...
            case "remove":
              {
                ...
                const updatedConfig = await manager.prisma.levelConfig.updateMany({
                  where: { guildId: guildId },
                  data: { channelId: null },
                });
