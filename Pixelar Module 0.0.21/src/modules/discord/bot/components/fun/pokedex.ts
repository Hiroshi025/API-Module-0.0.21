...
const pokedexCommand: Precommand = {
  ...
  owner: false,
  cooldown: 15,
  aliases: ["pokedex-search", "pokedex-pokemon", "pokedex-info"],
  botpermissions: ["SendMessages"],
  permissions: ["SendMessages"],
  subcommands: ["pokedex info <name>"],
  async execute(client, message, args, prefix, lang, config) {
      ...
      case "info":
        {
          try {
            ...
            const response = await getResponse(`${config.apis.pokedex}${pokemon}`);
            ...
            message.channel.send({
              embeds: [
                new ErrorEmbed(message.guild.id)
                  ...
                  .setStackTrace((err as any).stack)
                  .setDescription(
                    [
