...
const helpCommand: Precommand = {
  ...
  async execute(client: any, message, args, prefix, lang, config) {
    const categorias = readdirSync(`${config.paths.discord}/bot/components`);
    if (!message.guild) return;
    if (!client.user) return;
    if (args[0]) {
      const comando =
        client.precommands.get(args[0].toLowerCase()) ||
        client.precommands.find((c: Precommand) => c.aliases && c.aliases.includes(args[0].toLowerCase()));
          ...
          return message.reply({
            embeds: [
              new ErrorEmbed(message.guild?.id as string)
                ...
                .setStackTrace((e as Error).stack as string),
            ],
          });
        });
      } else if (categoria) {
        const comandos_de_categoria = getCommandsFromFolder(`${config.paths.discord}/bot/components/${categoria}`);
        return message.reply({
          embeds: [
            new EmbedBuilder()
              .setTitle(`${categoria}`)
        ...
        .setFields(
          {
            ...
            value: [`Welcome to the help menu of our control client **${client.user.username}**!`].join("\n"),
            inline: false,
          },
          {
            name: "__**Client Information**__",
            value: stripIndent`
							**Developer:** ${client.config.bot.owners.map((owner: string) => userMention(owner)).join(" - ")}
          ...
          .addOptions(
            categorias.map((categoria) => {
              const objeto = {
                label: categoria,
                value: categoria,
                description: `There are ${readdirSync(`${config.paths.discord}/bot/components/${categoria}`).length} commands in this category`,
              };
              return objeto;
            })
          )
      ...
      const botones = new ActionRowBuilder<ButtonBuilder>().addComponents([
        new ButtonBuilder()
          ...
          .setEmoji(client.getEmoji(message.guild.id).back)
          .setLabel("Back")
          .setCustomId("Atrás"),
        new ButtonBuilder()
          .setStyle(ButtonStyle.Primary)
          .setLabel("Home")
          .setEmoji(client.getEmoji(message.guild.id).home)
          .setCustomId("Inicio"),
        new ButtonBuilder()
          .setStyle(ButtonStyle.Success)
          .setLabel("Forward")
          .setEmoji(client.getEmoji(message.guild.id).forward)
          .setCustomId("Avanzar"),
        new ButtonBuilder()
          .setLabel("Go back")
      ...
      const collector = mensaje_ayuda.createMessageComponentCollector({
        ...
        }) => i.isButton() || (i.isStringSelectMenu() && i.user && i.message.author.id == client.user?.id),
        time: 180e3,
      });
        ...
        async (interaction: {
          ...
          if (interaction.isButton()) {
            ...
            for (const seleccionado of interaction.values) {
              const comandos_de_categoria = getCommandsFromFolder(`${config.paths.discord}/bot/components/${seleccionado}`);
              const embed = new EmbedBuilder()
