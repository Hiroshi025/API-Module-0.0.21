...
import { labelColors, labelNames } from "./variables";
import { WinstonFile } from "./winston";
...
export async function logWithLabel(label: Labels | "custom", message: string, customName?: string) {
  if (label === "custom" && customName === undefined) {
    throw new ValidationError("Custom label name must be provided when using the custom label type.");
  }
...
colors based on the label provided. Here's a breakdown of what it does: */
  ...
  const logger = await WinstonFile(config.paths.logs);
  switch (label) {
    case "error":
  ...
  // configuracion del log en un canal ppor webhook
  const webhook = new WebhookClient({ url: config.bot.webhooks.console });
  if (!webhook) return;

  await webhook.send({
    content: codeBlock("yaml", message),
    username: `${config.name} - Console`
  });
}
